import { validateTargetUrl } from "../security/ssrf.ts";

export interface ProxyRequestOptions {
  request: Request;
  defaultProxyAgent?: string;
}

/**
 * Core Proxy Handler
 * Reusable across Bun server, Cloudflare Pages Functions, and Cloudflare Workers.
 */
export async function handleProxyCore(options: ProxyRequestOptions): Promise<Response> {
  const { request, defaultProxyAgent } = options;
  ...
  // Determine upstream proxy agent
  ...
  let activeProxyAgent: string | undefined = defaultProxyAgent;
  ...
  if (activeProxyAgent) {
    try {
      ...
      upstreamResponse = await fetch(parsedTarget.toString(), fetchOpts as RequestInit);
